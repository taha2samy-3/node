...
PR_BODY_TEMPLATE = """## Automated Dependency & Security Update

This automated pull request refreshes base image digests and locks Wolfi system package versions to their latest stable releases inside `{file_path}`.
...
---
*This pull request was auto-generated by the automated dependency lock pipeline.*
"""
...
def build_summary_table(updates):
    docker_count = sum(1 for info in updates.values() if info["type"] == "Docker Image")
    package_count = sum(1 for info in updates.values() if info["type"] == "Wolfi Package")

    rows = [
        "| Category | Count |",
        "| :--- | :---: |",
        f"| Docker Images | {docker_count} |",
        f"| Wolfi Packages | {package_count} |",
        f"| **Total** | **{len(updates)}** |",
    ]
    return "\n".join(rows)


def build_details_table(updates):
    rows = [
        "| Variable | Type | Target | New Value |",
        "| :--- | :--- | :--- | :--- |",
    ]
    for var_name, info in updates.items():
        rows.append(f"| `{var_name}` | {info['type']} | `{info['target']}` | `{info['value']}` |")
    return "\n".join(rows)


def render_pr_body(file_path, updates):
    return PR_BODY_TEMPLATE.format(
        file_path=file_path,
        timestamp=datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC"),
        total_updates=len(updates),
        summary_table=build_summary_table(updates),
        details_table=build_details_table(updates),
    )
...
def create_pull_request(file_path, updates):
    ...
    body = render_pr_body(file_path, updates)
