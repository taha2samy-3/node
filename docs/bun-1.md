...
# :simple-bun: Bun {{ python_3_12.prod.tags[0].split(':')[-1].split('-')[0] }}
...
{% set flavors = [
  ('dev', 'Development Builder', 'material-toolbox-outline', 'Full Developer Suite', 'Contains the full Bun runtime and essential build utilities for development and testing.'),
  ('prod', 'Production Runtime (Scratch)', 'material-shield-check', 'Hardened Minimal Runtime', 'Ultra-minimal, secure Bun runtime directly from scratch with no extra tools or shell.')
] %}
...
=== ":{{ icon }}: {{ title }}"
    ...
    **Pull by Version Tag**
    ```bash
    docker pull {{ bun_1[key].tags[0] }}
    ```

    **Pull by Floating Tag**
    ```bash
    docker pull {{ bun_1[key].tags[1] }}
    ```
    ...
    === ":material-shield-search: Vulnerability Scan"
        ...
        {% set scan_data = bun_1[key]['security'] %}
        ...
        | **`{{ p['Name'] }}`** | `{{ p['Version'] }}` | {{ p['Licenses'] | join(', ') if p['Licenses'] is iterable and p['Licenses'] is not string else p['Licenses'] | default('N/A') }} | {% if res['Class'] == 'os-pkgs' %}:material-linux: System (Wolfi){% else %}:simple-bun: Bun Package{% endif %} |
              {% endfor -%}
            {% endif -%}
          {% endfor -%}
        {% endif -%}
<br>
    === ":fontawesome-brands-docker: Docker CIS"
        ...
        {% set cis_data = bun_1[key]['docker-csi'] %}
