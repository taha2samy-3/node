...
# :fontawesome-brands-node-js: Node.js {{ node_24.prod.tags[0].split(':')[-1].split('-')[0] }} LTS
...
{% set flavors = [
  ('dev', 'Development Builder (with NPM)', 'material-toolbox-outline', 'Full Developer Suite', 'Contains Node.js and NPM for building packages and running tests.'),
...
=== ":{{ icon }}: {{ title }}"
    ...
    **Pull by Version Tag**
    ```bash
    docker pull {{ node_24[key].tags[0] }}
    ```

    **Pull by Floating Tag**
    ```bash
    docker pull {{ node_24[key].tags[1] }}
    ```
    ...
    === ":material-shield-search: Vulnerability Scan"
        ...
        {% set scan_data = node_24[key]['security'] %}
...
<br>
    === ":fontawesome-brands-docker: Docker CIS"
        ...
        {% set cis_data = node_24[key]['docker-csi'] %}
