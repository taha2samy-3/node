...
# :fontawesome-brands-node-js: Node.js {{ node_20.prod.tags[0].split(':')[-1].split('-')[0] }} LTS
...
=== ":{{ icon }}: {{ title }}"
    ...
    **Pull by Version Tag**
    ```bash
    docker pull {{ node_20[key].tags[0] }}
    ```

    **Pull by Floating Tag**
    ```bash
    docker pull {{ node_20[key].tags[1] }}
    ```
    ...
    === ":material-shield-search: Vulnerability Scan"
        ...
        {% set scan_data = node_20[key]['security'] %}
...
<br>
    === ":fontawesome-brands-docker: Docker CIS"
        ...
        {% set cis_data = node_20[key]['docker-csi'] %}
