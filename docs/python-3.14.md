...
# :fontawesome-brands-python: Python {{ python_3_14.prod.tags[0].split(':')[-1].split('-')[0] }}
...
=== ":{{ icon }}: {{ title }}"
    ...
    **Pull by Version Tag**
    ```bash
    docker pull {{ python_3_14[key].tags[0] }}
    ```

    **Pull by Floating Tag**
    ```bash
    docker pull {{ python_3_14[key].tags[1] }}
    ```
    ...
    === ":material-shield-search: Vulnerability Scan"
        ...
        {% set scan_data = python_3_14[key]['security'] %}
...
<br>
    === ":fontawesome-brands-docker: Docker CIS"
        ...
        {% set cis_data = python_3_14[key]['docker-csi'] %}
