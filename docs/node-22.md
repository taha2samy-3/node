...
# :fontawesome-brands-node-js: Node.js {{ node_22.prod.tags[0].split(':')[-1].split('-')[0] }} LTS
...
=== ":{{ icon }}: {{ title }}"
    ...
    **Pull by Version Tag**
    ```bash
    docker pull {{ node_22[key].tags[0] }}
    ```

    **Pull by Floating Tag**
    ```bash
    docker pull {{ node_22[key].tags[1] }}
    ```
    ...
    === ":material-shield-search: Vulnerability Scan"
        ...
        {% set scan_data = node_22[key]['security'] %}
...
<br>
    === ":fontawesome-brands-docker: Docker CIS"
        ...
        {% set cis_data = node_22[key]['docker-csi'] %}
